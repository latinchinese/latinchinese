...
document.getElementById('inquiryForm').addEventListener('submit', function(e) {
    ...
    if (!name || !whatsapp) {
        alert(currentLang === 'es' ? 'Por favor completa tu nombre y WhatsApp.' : '请填写姓名和WhatsApp');
        return;
    }
    const subject = `Consulta sobre ${productName}`;
    const body = `Nombre: ${name}%0AWhatsApp: ${whatsapp}%0AProducto: ${productName}%0AMensaje: ${msg}`;
...
function toggleLang() {
    ...
    if (currentLang === 'es') {
        footerText.innerHTML = '¿Eres fabricante? <a href="upload_guide.html">Sube tus productos aquí</a>';
        document.getElementById('modal-title').innerText = 'Solicitar cotización';
    } else {
        footerText.innerHTML = '您是工厂吗？<a href="upload_guide.html">免费上传您的产品</a>';
        document.getElementById('modal-title').innerText = '询价';
    }
}
